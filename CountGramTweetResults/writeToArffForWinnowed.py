...
file = open('C:/UMD/Research/sparseArffWinnowed3up.txt','at')
'''
...
winnowedLoc = ("C:/UMD/Research/CountGramTweetResults/winnowed3up.xlsx")
...
sorterForGram = {}
sorterForTopic = {}
file.write('@ATTRIBUTE class {1,2}\n')
file.write('@DATA\n')
file.write('{')
for i in range(1, winnowedSheet.nrows):
    tweetID = int(winnowedSheet.cell_value(i,0))
    topicNumber = int(winnowedSheet.cell_value(i, 1))
    gramID = int(winnowedSheet.cell_value(i, 2))
    gramCount = int(winnowedSheet.cell_value(i,3))
    if tweetID != tweetIDS[indexForTweetID]:
        ...
        file.write(' ')
#        file.write('\'topic')
        file.write(str(topicNumTracker))
        file.write('}\n')
        file.write('{')
