...
cur = myDB.cursor()
putWordToDB = "insert into trigram (triword) values (%s);"
...
def ngramWithoutStopPunct(tokens):
    ...
    for i in range(len(twoWordList) - 2):
        twoTokens = []
        if twoWordList[i+1] - twoWordList[i] == 1 and twoWordList[i+2] - twoWordList[i+1] == 1:
